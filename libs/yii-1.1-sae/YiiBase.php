...
class YiiBase
{
	...
	public static function autoload($className)
	{
        ...
        {
            $arr = array('CStatePersister','CFileLogRoute','CMemCache','CAssetManager','CDbCommand','CDbConnection','CWebApplication');
            foreach($arr as $name)
            {
                if(substr(self::$_coreClasses[$name],0,4) != '/sae')
                    self::$_coreClasses[$name] = '/sae'.self::$_coreClasses[$name];
            }
            self::$_coreClasses['SAEAssetsAction'] = '/sae/web/SAEAssetsAction.php';
            self::$_coreClasses['SAECommon'] = '/sae/SAECommon.php';
        }
