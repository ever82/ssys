 ...
 * @param 
 */
$$.O.prototype.getRoutedConfig=function(path,configType,way){
  if(path===null){
    return null;
  }
  
  var routedConfigValues;
  if(way=="list"||way=="concat"){
    routedConfigValues=[];
  }
  //这时configType是string,它是独立出来的配置
  var routedConfigs=this[configType+"Configs"]||{};
  var routedConfigRules=this[configType+"ConfigRules"];
  var routedConfigValue=routedConfigs[path];
  var matched=null;
  if(!routedConfigValue&&routedConfigRules){
    for(var i=0,l=routedConfigRules.length;i<l;i++){
      var rule=routedConfigRules[i];
      var regex=rule[0];
      matched=path.match(regex);
      if(matched){
        routedConfigValue=rule[1];
        if(routedConfigValue){
          break;
        }
      }
    }
  ...
  if(routedConfigValue){
    return this.parseConfig(routedConfigValue,matched);
  }
};
