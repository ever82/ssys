$$.App=$$.O.createSubclass({
    ...
    init:function(domnode,state){
      ...
      console.debug("app init, domnode=",domnode,"state=",state);
    },
    ...
    getViewByName:function(fullname){
      ...
      for(var i=0,l=names.length;i<l;i++){
        ...
        if(view){
          ...
          throw "getViewByName("+fullname+")失败了, "+oldView.fullname+"的elements中没找到"+names[i-1];
        }
      }
      return view;
    },
    ...
    addTestModule:function(name,module){
      return this.testModules[name]=module;
    },
    createCurrentUser:function(){
      this.currentUser=$$.User.create(this);
    },
    setLayout:function(domnode,state){
      var layout=this.self.Layout.create(this,domnode);
      layout.createdAt=this.initAt;
      state=state||$$.getStorage('initState');
      if(state===undefined){
    ...
    setState:function(state,from,forceUnlock){
      return this.layout.setState(state,from,forceUnlock);
    },
    ...
    Layout:$$.View.createSubclass({
        mg_elementConfigs:{
          deviceDetector:['Html','<div><div class="device-xs visible-xs"></div><div class="device-sm visible-sm"></div><div class="device-md visible-md"></div><div class="device-lg visible-lg"></div></div>']
        },
        mg_showConfigs:{
          index:['p_index']
        },
