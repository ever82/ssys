$$.View.Playable=$$.View.createSubclass({
    ...
    getLabelTitleByStageState:function(stageState){
      ...
      if(stage){
        ...
        var labelTitle=stage.getStateConfig(state,0);
        if(labelTitle){
    ...
    getStateShow:function(state){
      var showConfig=this.getStateConfig(state,2);
      if(showConfig){
        var show=showConfig;
    ...
    getStateTips:function(state){
      var tips=this.getStateConfig(state,'tips');
      return tips;
    },
    playingTipIndex:-1,
    ...
    getPreloadByStageState:function(stageState){
      ...
      if(stage){
        var state=stageState[1]||stage.defaultState;
        //console.debug(this.fullname,"已经找到最终舞台=",stage.fullname,"和它上面在表演的节目=",state);
        var preload=stage.getStateConfig(state,'preload');
    ...
    getSecondsByStageState:function(stageState){
      ...
      if(stage){
        var state=stageState[1]||stage.defaultState;
        //console.debug(this.fullname,"已经找到最终舞台=",stage.fullname,"和它上面在表演的节目=",state);
        var seconds=stage.getStateConfig(state,'seconds');
    ...
    getNextStepsByStageState:function(stageState){
      ...
      if(stage){
        ...
        var nextSteps=stage.getStateConfig(state,'nextSteps');
