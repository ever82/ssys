$$.View.Slider=$$.View.createSubclass({
    ...
    beforeInit:function(title,options){
      ...
      this.title=title;
      this.min=options.min=parseFloat(options.min);
      this.max=options.max=parseFloat(options.max);
      this.step=options.step=parseFloat(options.step);
      this.unit=options.unit||'';
      this.onSlide=options.onSlide;
      this.imgSrc=options.imgSrc;
      this.precision=options.precision||0;
      this.sliderValue=options.defaultValue||0;//this.find(".sliderValue");
      ssys.getCss(ssys.sresBaseUrl+"lib/bootstrap/slider/css/slider.css");
    },
    get_imgConfig:function(){
      if(!this.imgSrc){
        return ''; 
      }else{
    ...
    updateInputData:function(){
      return this.inputData=this.slider.val();
    }
